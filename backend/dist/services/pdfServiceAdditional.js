...
const generateLabResultPDF = async (labResult, patient) => {
    ...
    const renderDynamicSection = (lab) => {
        const template = lab.template;
        const dyn = (lab.dynamicData || {});
        ...
        for (const f of sorted) {
            ...
            if (v === null || v === undefined || v === '')
                continue;
            const unit = dyn[`${f.key}Unit`] ?? f.unit ?? '';
            const refLow = dyn[`${f.key}RefLow`] ?? f.refLow;
            const refHigh = dyn[`${f.key}RefHigh`] ?? f.refHigh;
            const flag = dyn[`${f.key}Flag`];
            if (f.type === 'NUMBER') {
