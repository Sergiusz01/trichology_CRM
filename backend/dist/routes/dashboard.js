...
router.get('/', auth_1.authenticate, async (req, res, next) => {
    try {
        const [patients, consultations, emails, visits, labResults, scalpPhotos, carePlans, upcomingVisits, weeklyRevenue] = await Promise.all([
            ...
            prisma_1.prisma.scalpPhoto.findMany({
                select: {
                    id: true,
                    patientId: true,
            ...
            prisma_1.prisma.visit.findMany({
                where: {
                    data: {
                        gte: (0, warsawTz_1.startOfTodayWarsaw)(),
                    },
                    status: 'ZAPLANOWANA',
                },
                include: {
        ...
        const patientsWithoutConsultation = patients.filter((p) => !consultations.some((c) => c.patientId === p.id)).length;
        ...
        visits.forEach((visit) => {
            ...
            const patient = visit.patient || patients.find((p) => p.id === visit.patientId);
            // Dodanie wizyty
            activities.push({
        ...
        labResults.forEach((labResult) => {
            ...
            const patient = labResult.patient || patients.find((p) => p.id === labResult.patientId);
        ...
        scalpPhotos.forEach((photo) => {
            const createdAt = photo.createdAt instanceof Date ? photo.createdAt : new Date(photo.createdAt);
            const patient = photo.patient || patients.find((p) => p.id === photo.patientId);
            activities.push({
        ...
        carePlans.forEach((carePlan) => {
            ...
            const patient = carePlan.patient || patients.find((p) => p.id === carePlan.patientId);
            // Dodanie planu
            activities.push({
        ...
        const patientsWithoutConsultations = patients.filter((p) => !consultations.some((c) => c.patientId === p.id));
        res.json({
            stats: {
