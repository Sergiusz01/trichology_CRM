...
const labResultSchema = zod_1.z.object({
    ...
    dynamicData: zod_1.z.record(zod_1.z.unknown()).optional(),
});
...
router.post('/', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (data.templateId && data.dynamicData) {
            const t = await prisma_1.prisma.labResultTemplate.findFirst({
            ...
            const dynamicData = (0, labResults_1.calculateDynamicDataFlags)(fields, data.dynamicData);
            ...
            await (0, auditService_1.writeAuditLog)(req, { action: 'CREATE_LAB_RESULT', entity: 'LabResult', entityId: labResult.id });
...
router.put('/:id', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (data.templateId && data.dynamicData) {
            ...
            const fields = t.fields || [];
            const dynamicData = (0, labResults_1.calculateDynamicDataFlags)(fields, data.dynamicData);
            const labResult = await prisma_1.prisma.labResult.update({
                where: { id },
                data: {
            ...
            await (0, auditService_1.writeAuditLog)(req, { action: 'UPDATE_LAB_RESULT', entity: 'LabResult', entityId: labResult.id });
...
router.post('/:id/restore', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const restoredLabResult = await prisma_1.prisma.labResult.update({
            where: { id },
