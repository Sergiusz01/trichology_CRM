...
import { verifyActionToken } from '../utils/appointmentToken';
...
const buildResultPage = (title: string, message: string, color: string, isError = false): string => `
<!DOCTYPE html>
<html lang="pl">
<head>
  <meta charset="UTF-8">
...
router.get('/', async (req, res) => {
  ...
  if (!token || typeof token !== 'string') {
    return res.status(400).send(buildResultPage(
      'Nieprawidłowy link',
      'Link jest nieprawidłowy lub niekompletny. Sprawdź treść wiadomości email.',
      '#FF3B30',
      true,
    ));
  }

  let payload: { visitId: string; action: 'confirm' | 'cancel' | 'reschedule' };
  try {
    payload = verifyActionToken(token);
  } catch (err) {
    if (err instanceof jwt.TokenExpiredError) {
      return res.status(410).send(buildResultPage(
        'Link wygasł',
        'Ten link działał przez 48 godzin i już wygasł. Skontaktuj się z kliniką, jeśli chcesz zmienić status wizyty.',
        '#FF9500',
        true,
      ));
    }
    return res.status(400).send(buildResultPage(
      'Nieprawidłowy link',
      'Link jest nieprawidłowy lub został zmodyfikowany. Sprawdź treść wiadomości email.',
      '#FF3B30',
      true,
    ));
  }
  ...
  try {
    ...
    if (!visit) {
      return res.status(404).send(buildResultPage(
        'Wizyta nie znaleziona',
        'Nie znaleziono wizyty powiązanej z tym linkiem.',
        '#FF3B30',
        true,
      ));
    }
    ...
    if (visit.status === newStatus) {
      ...
      return res.send(buildResultPage(
        title,
        `Akcja "${ACTION_LABELS[action]}" dla wizyty ${visitDateFormatted} jest już zarejestrowana.`,
        color,
      ));
    }
    ...
    if (!allowedSourceStatuses.includes(visit.status)) {
      return res.send(buildResultPage(
        'Akcja niedostępna',
        `Status tej wizyty wynosi "${visit.status}" i nie można już zmieniać jej statusu przez link email.`,
        '#FF9500',
        true,
      ));
    }
    ...
    return res.status(500).send(buildResultPage(
      'Błąd serwera',
      'Wystąpił nieoczekiwany błąd. Spróbuj ponownie lub skontaktuj się z kliniką.',
      '#FF3B30',
      true,
    ));
  }
...
async function sendSpecialistNotification(
  visit: any,
  action: string,
...
<div class="wrap">
  ...
  for (const specialist of specialists) {
    try {
      await sendEmail({ to: specialist.email, subject, html });
    } catch (e) {
