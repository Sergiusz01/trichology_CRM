...
function getLink(
  ...
  maps?: {
    visitPatient?: Map<string, string>;
    labPatient?: Map<string, string>;
    carePlanPatient?: Map<string, string>;
  }
): string {
  if (!entityId) return '';
  ...
  if (entity === 'Consultation') return `/consultations/${entityId}`;
  if (entity === 'LabResult') {
...
router.get('/', authenticate, async (req: AuthRequest, res, next) => {
  try {
    ...
    const visitIds = Array.from(
      new Set(logs.filter((l) => l.entity === 'Visit' && l.entityId).map((l) => l.entityId as string))
    );
    const labIds = Array.from(
      new Set(logs.filter((l) => l.entity === 'LabResult' && l.entityId).map((l) => l.entityId as string))
    );
    const carePlanIds = Array.from(
      new Set(logs.filter((l) => l.entity === 'CarePlan' && l.entityId).map((l) => l.entityId as string))
    );
